(function (window, createjs, opspark, _) {
  ...
  function padCollide(pad, drection) {
    // check if the ball's X matches the paddle's X
    if (
      ball.x + ball.radius * drection === pad.x * drection ||
      ball.x < pad.x * -drection
    ) {
      // check if the ball's y falls within the bounds of the paddle
      ...
      // if the ball has surpassed the paddle, end the game
