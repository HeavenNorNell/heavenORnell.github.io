...
(function (window, createjs, opspark, dreamscape) {
  ...
  const // setup the standard demo
    ...
    side = 50, // the radius of our two circles
    squareUp = new createjs.Shape(), // the up state: the mouse is NOT intersecting
    squareOver = new createjs.Shape(); // the over state: the mouse IS intersecting
  ...
  squareUp.graphics.beginFill("blue").drawRect(0, 0, side, side);
  squareOver.graphics.beginFill("red").drawCircle(0, 0, side, side);
  squareOver.alpha = 0;
  squareUp.x = squareOver.x = canvas.width / 3;
  squareUp.y = squareOver.y = canvas.height / 3;
  squareActual = {
    x: squareUp.x - side / 2,
    y: squareUp.y - side / 2,
  };
   ...
   */
  const textfield1 = new createjs.Text("Distance: ", "20px Arial", "#BBB"),
    textBounds1 = textfield1.getBounds();

  textfield1.x = (canvas.width - textBounds1.width) / 2;
  textfield1.y = canvas.height / 2 + 50;

  const textfield2 = new createjs.Text("Distance: ", "20px Arial", "#BBB"),
    textBounds2 = textfield2.getBounds();

  textfield2.x = (canvas.width - textBounds2.width) / 3;
  textfield2.y = canvas.height / 3 + 50;
  ...
  function update(event) {
    ...
    distance1 = dreamscape.phyz.calculateDistance(shapeUp, mouse);
    distance2 = dreamscape.phyz.calculateDistance(squareActual, mouse);
    /*
     * TODO 6: Check if the mouse is within the area of shapeUp, and set the
     * alpha property of shapeUp accordingly:
     */

    shapeOver.alpha = distance1 < radius ? 1 : 0;

    /*
     * Update the textfield with the current distance between the mouse and
     *the edge of the shapeUp
     */
    updateText(textfield1, `Distance: ${Math.round(distance1)}px`);
    updateText(textfield2, `Distance: ${Math.round(distance2)}px`);
  }
...
})(window, window.createjs, window.opspark, window.dreamscape);
